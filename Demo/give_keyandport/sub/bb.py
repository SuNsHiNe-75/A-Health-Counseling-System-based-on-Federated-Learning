...
if __name__ == '__main__':
    ...
    port = 1248
    address = "10.0.0.31"#serveraddr
    myaddr=getaddresss()
    portdata={}
    portdata['check'] = {'cc':"gk"}
    portdata['port'] = {'p':myport}
    portdata['port'] = {'p':myport}
    portdata['addr'] = {'ad':myaddr}
    ...
    if choice_str == "Y":
        ...
        for i in range(len(clientport)):
            sg.one_line_progress_meter('實時進度條', i + 1, len(clientport), 'Finding User')
        ...
        with open('./weight-loss-master/scores.txt', newline='') as csvfile:
            rows = csv.reader(csvfile)
            for row in rows:
                myscore.append(row)
        mynamelist=[]
        ...
        namelist=[]
        proc = subprocess.Popen([ "python3","progress1.py"])

        for i in range(len(sameclient)):
            port = int(clientport[sameclient[i]])
            address = (clientaddr[sameclient[i]], port)
            filename = "data.json"
            sleep(0.101)
            send(address, filename)
            port = 1235
            address = "127.0.0.1"
            recvived(myaddr, port)
            anss=loadAnswer()
            pk=anss['public_key']
            pubkey= paillier.PaillierPublicKey(n=int(pk['n']))
            ...
            print(ressocelist)
        proc.terminate()
        proc = subprocess.Popen([ "python3","serlize.py"])
        ...
        with open('port.json', 'w') as file:
            ...
            file.close()
        filename="port.json"
        addresss = ("10.0.0.31", 1248)#server
        send(addresss, filename)

        proc = subprocess.Popen([ "python3","progress.py"])
        ...
        for sss in range(len(resnamelist)-len(mynamelist)):
            sleep(0.011)
            newsocre.append((resnamelist[sss+ss+1]+ ","+str(float(finalans[sss+ss+1][0])/1000))+",new")
        ...
        with open('scnew.csv', 'w') as file:
            for ii in range(len(newsocre)):
                file.write("%s\n" % (newsocre[ii]))
        os.system("python3 qweqwe.py")
        os.system("python3 sss.py")
'''
    else:
